...
function initialise() {
	...
	if (initStatus === false) {
		...
		hideEndPageChangeButtons();	//At the extremes - start and end of the story
		timerIDs = [];
		showPage(pgOn, "load");
	}
}
...
function showPage(i, evnt) {
	...
	AudioPlayer.pause();	//In case it's running
	pageID = "p" + i;
	pagenumber = " ";
	...
	//If it's page 0 from onload event, play the audio automatically
	if (evnt === "load") {
		document.getElementById(pageID).getElementsByClassName("button")[0].childNodes[0].click();
	}
}
...
function playAudio(file, time, pageNo, lineNo) {
	...
	//Load and start the audio file and the progress bar
	player.src = file;
	player.play();
...
function runProgBar(pauseListWrap) {
	...
	var pauseListLine = [];
	var startDelay = 900;	//applied to first line
